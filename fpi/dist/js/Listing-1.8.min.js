"use strict";
(self["webpackChunktheme"] = self["webpackChunktheme"] || []).push([["Listing"],{

/***/ "./src/js/components/LazyLoad.js":
/*!***************************************!*\
  !*** ./src/js/components/LazyLoad.js ***!
  \***************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": () => (/* binding */ LazyLoad)
/* harmony export */ });
/* harmony import */ var gsap__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! gsap */ "./node_modules/gsap/index.js");
/* harmony import */ var gsap_ScrollTrigger__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! gsap/ScrollTrigger */ "./node_modules/gsap/ScrollTrigger.js");
/* provided dependency */ var $ = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, _toPropertyKey(descriptor.key), descriptor); } }
function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function _defineProperty(obj, key, value) { key = _toPropertyKey(key); if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
function _toPropertyKey(t) { var i = _toPrimitive(t, "string"); return "symbol" == _typeof(i) ? i : i + ""; }
function _toPrimitive(t, r) { if ("object" != _typeof(t) || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != _typeof(i)) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }


gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.registerPlugin(gsap_ScrollTrigger__WEBPACK_IMPORTED_MODULE_1__.ScrollTrigger);
var LazyLoad = /*#__PURE__*/_createClass(function LazyLoad() {
  var _this = this;
  _classCallCheck(this, LazyLoad);
  _defineProperty(this, "init", function () {
    _this.setDomMap();
    _this.bindEvents();
  });
  _defineProperty(this, "setDomMap", function () {
    _this.window = $(window);
    _this.body = $("body");
  });
  _defineProperty(this, "bindEvents", function () {
    _this.initLazyLoad();
  });
  _defineProperty(this, "initLazyLoad", function () {
    var lazyImageView = document.querySelectorAll(".lazy-image");
    if (lazyImageView.length) {
      lazyImageView.forEach(function (image) {
        var loadImage = function loadImage() {
          if (!image.classList.contains("loaded")) {
            image.classList.add("loaded");
            var imgDataSrc = image.getAttribute("data-src");
            if (imgDataSrc) {
              image.setAttribute("src", imgDataSrc);
              image.onload = function () {
                image.classList.add("loaded-full-image");
              };
            }
          }
        };
        var offset = image.getAttribute("data-offset");
        var defaultOffset = 700;
        var lazyImageViewTimeline = gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.timeline({
          scrollTrigger: {
            trigger: image,
            start: "top bottom+=".concat(offset ? offset : defaultOffset),
            onEnter: loadImage
          }
        });
      });
      gsap_ScrollTrigger__WEBPACK_IMPORTED_MODULE_1__.ScrollTrigger.refresh();
    }
    var lazyVideoView = document.querySelectorAll(".lazy-video");
    if (lazyVideoView.length) {
      lazyVideoView.forEach(function (video) {
        var videoDataSrc = video.getAttribute("data-src");
        var newSource = document.createElement("source");
        newSource.setAttribute("src", videoDataSrc);
        var loadVideo = function loadVideo() {
          if (!video.classList.contains("loaded")) {
            video.classList.add("loaded");
            video.appendChild(newSource);
            video.removeAttribute("data-src");
          }
        };
        var offset = video.getAttribute("data-offset");
        var defaultOffset = 700;
        var videoTimeline = gsap__WEBPACK_IMPORTED_MODULE_0__.gsap.timeline({
          scrollTrigger: {
            trigger: video,
            start: "top bottom+=".concat(offset ? offset : defaultOffset),
            onEnter: loadVideo
          }
        });
      });
      gsap_ScrollTrigger__WEBPACK_IMPORTED_MODULE_1__.ScrollTrigger.refresh();
    }
  });
  this.init();
});


/***/ }),

/***/ "./src/js/components/Listing.js":
/*!**************************************!*\
  !*** ./src/js/components/Listing.js ***!
  \**************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": () => (/* binding */ Listing)
/* harmony export */ });
/* harmony import */ var _LazyLoad__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./LazyLoad */ "./src/js/components/LazyLoad.js");
/* provided dependency */ var $ = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, _toPropertyKey(descriptor.key), descriptor); } }
function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
function _toPropertyKey(t) { var i = _toPrimitive(t, "string"); return "symbol" == _typeof(i) ? i : i + ""; }
function _toPrimitive(t, r) { if ("object" != _typeof(t) || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != _typeof(i)) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }

var Listing = /*#__PURE__*/function () {
  function Listing() {
    _classCallCheck(this, Listing);
    this.init();
    this.bindEvents();
  }
  return _createClass(Listing, [{
    key: "init",
    value: function init() {
      this.listItemContainer = this.parent.find(".list-item-container");
      // this.ccmToken = this.parent.find('[name="ccm_token"]');
      this.loadMore = this.parent.find(".load-more");
      this.loader = this.parent.find(".loader");
      this.noResult = this.parent.find("#no-result");
      this.page = 1;
      this.autoLoad = true;
    }
  }, {
    key: "bindEvents",
    value: function bindEvents() {}
  }, {
    key: "initOnClickLoadMore",
    value: function initOnClickLoadMore() {
      this.loadMore.on("click", this.nextPage.bind(this));
    }
  }, {
    key: "initOnScrollLoadMore",
    value: function initOnScrollLoadMore() {
      $(window).scroll(this.windowScroll.bind(this));
    }
  }, {
    key: "windowScroll",
    value: function windowScroll(e) {
      if (this.autoLoad && this.isAtBottom()) {
        this.autoLoad = false;
        this.nextPage(e);
      }
    }
  }, {
    key: "isAtBottom",
    value: function isAtBottom() {
      var scroll = this.listItemContainer.scrollTop() + this.listItemContainer.innerHeight() + 100;
      var scrollHeight = this.listItemContainer[0].scrollHeight;
      return scroll >= scrollHeight;
    }
  }, {
    key: "delayFilter",
    value: function delayFilter() {
      var _this = this;
      clearInterval(this.timer);
      this.timer = setTimeout(function () {
        _this.applyFilter();
      }, 500);
    }
  }, {
    key: "nextPage",
    value: function nextPage(e) {
      e.preventDefault();
      this.addPage();
      this.toggleNoResults(true);
      this.showLoader();
      this.requestData();
    }
  }, {
    key: "requestData",
    value: function requestData() {
      $.ajax({
        dataType: "json",
        type: "GET",
        cache: false,
        data: this.formParams(),
        url: ajaxurl,
        success: this.bindData.bind(this),
        error: this.handleError.bind(this)
      });
    }
  }, {
    key: "formParams",
    value: function formParams() {
      var data = {};
      data["page"] = this.page;
      // data["ccm_token"] = this.ccmToken.val();

      return data;
    }
  }, {
    key: "addPage",
    value: function addPage() {
      this.page++;
    }
  }, {
    key: "resetPage",
    value: function resetPage() {
      this.page = 1;
    }
  }, {
    key: "applyFilter",
    value: function applyFilter() {
      this.resetPage();
      this.clearContainer();
      this.toggleNoResults(true);
      this.showLoader();
      this.setHistory();
      this.requestData();
    }
  }, {
    key: "clearContainer",
    value: function clearContainer() {
      this.listItemContainer.empty();
    }
  }, {
    key: "showLoader",
    value: function showLoader() {
      this.loader.show();
    }
  }, {
    key: "hideLoader",
    value: function hideLoader() {
      this.loader.hide();
    }
  }, {
    key: "bindData",
    value: function bindData(response) {
      this.listItemContainer.append(response.data);
      this.autoLoad = response.hasNextPage;
      this.hideLoader();
      this.toggleLoadMore(response.hasNextPage);
      this.toggleNoResults(response.success);
      this.additionalDataBind(response);
      this.callbackFn();
      new _LazyLoad__WEBPACK_IMPORTED_MODULE_0__["default"]();
    }
  }, {
    key: "setHistory",
    value: function setHistory() {
      if (window.history.pushState) {
        var url = "?";
        var params = [];
        var data = this.formParams();
        for (var key in data) {
          if (key === "page" || key === "ccm_token" || key === "action" || key === "post_type") continue;
          params.push("".concat(key, "=").concat(data[key]));
        }
        url += params.join("&");
        window.history.pushState("", "", url);
      }
    }
  }, {
    key: "handleError",
    value: function handleError(textStatus, errorThrown) {
      // window.location.reload();
    }
  }, {
    key: "toggleLoadMore",
    value: function toggleLoadMore(flag) {
      flag ? this.loadMore.show() : this.loadMore.hide();
    }
  }, {
    key: "toggleNoResults",
    value: function toggleNoResults(flag) {
      flag ? this.noResult.hide() : this.noResult.show();
    }
  }, {
    key: "callbackFn",
    value: function callbackFn() {}
  }, {
    key: "additionalDataBind",
    value: function additionalDataBind(response) {}
  }]);
}();


/***/ })

}]);
//# sourceMappingURL=Listing-1.8.min.js.map