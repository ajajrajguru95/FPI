    ...
    ?>
    <a class="flex flex-col items-start group cursor-pointer mb-5 !opacity-100 translate-y-0 [&:nth-child(3n-1)]:translate-y-5 fade-up <?= ($linkedin_profile) ? 'cursor-default':'';?>" href="<?php if ($linkedin_profile) { echo $linkedin_profile; } else { ?>javascript:void(0);<?php } ?>" target="<?php if ($linkedin_profile) {?>_target <?php } else { ?>_self<?php } ?>">
        <div class="flex overflow-hidden relative flex-col items-end self-stretch w-full aspect-square ">
            ...
            <?php endif;?>
            <img class="object-cover scale-100 duration-700 transition-all group-hover:scale-110 h-full" loading="lazy" src="<?= $post_featured_img; ?>" alt="<?= the_title(); ?>" />
        </div>
