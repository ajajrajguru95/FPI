...
<?php if (have_posts()) : ?>
    ...
    $postType = 'profiles';
    $taxonomyHandle = 'profiles_category';
    ?>

        <!-- Page banner -->
        <?php include(locate_template('template-parts/parts/short_banner.php', false, false)); ?>

        <section class="common-listing py-[6.25rem] max-md:py-12 px-16 max-xl:px-5  fade-in" data-type="<?php echo $postType; ?>">
            <div class="max-container">
                <div class="filter-block mb-14 max-md:mb-8 flex max-sl:flex-col gap-5 justify-between">
                    ...
                    <div class="select-block flex max-md:flex-col  gap-5 md:h-16">
                        <?php
                        get_template_part('template-parts/filters/category', 'post', array('taxonomy_handle' => 'profiles_category'));
